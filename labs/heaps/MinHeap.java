...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	private void moveItem(int from, int to) {
		Decreaser<T> d_from = array[from];
		Decreaser<T> d_to = array[to];
		d_from.loc = to;
		d_to.loc = from;
		array[to] = d_from;
		array[from] = d_to;	
	...
	private void heapify(int where) {
		...
		//
		if ((where*2+1)<=size){
			T l = array[where*2].getValue();
			T r = array[where*2+1].getValue();
			ticker.tick(2);
			if(array[where].getValue().compareTo(l)>0 || array[where].getValue().compareTo(r)>0){
				if (l.compareTo(r)<0){
